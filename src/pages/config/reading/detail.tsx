...
const ReadingConfigDetail: React.FC = () => {
  ...
  const typeOptions = [
    ...
    { label: "选词填空题", value: "choose_word" },
    { label: "单选题", value: "choice" },
    { label: "多选题", value: "multi_choice" },
    { label: "细节匹配题", value: "matching" },
    { label: "段落匹配题", value: "paragraph_matching" },
    { label: "判断题", value: "true_or_false" },
    { label: "标题题", value: "heading" },
  ];
      ...
      <DynamicForm
        ...
        formItemsConfig={[
          ...
          },
          {
            ...
            type: "TableEdit",
            visible: (field: any) =>
              ["matching", "choose_word", "heading"].includes(
                String(formData[index].items[field.name].type)
              ),
            addText: "新增选项",
            rowKey: "label",
          ...
          },
          {
            ...
            rowKey: "no",
            visible: (field: any) =>
              formData[index].items[field.name].type &&
              formData[index].items[field.name].type !== "heading",
            addText: "新增题目",
            addTableRow: (field: any) => {
              const arr = [...formData];
              let length: number = 0;
              ...
              },
              {
                ...
                visible: ({ field }: any) =>
                  [
                    ...
                    "paragraph_matching",
                    "true_or_false",
                  ].includes(String(formData[index].items[field.name].type)),
              },
              {
                title: "选项",
                key: "options",
  ...
  const generateArticle = () => {
    return (
      <Form.Item label="文章内容" name="article">
        <ReactQuill
          ref={editorRef}
          theme="snow"
          placeholder="贴入文章内容，请根据文章格式自行调整"
          modules={{
            toolbar: {
        ...
        <Alert
          className="mt-10"
          message="如该篇文章包含heading题，则在段落头部输入题号再点击插入答题框，段落之间请用空行隔开。"
          type="info"
        />
      </Form.Item>
    );
...
export default ReadingConfigDetail;
